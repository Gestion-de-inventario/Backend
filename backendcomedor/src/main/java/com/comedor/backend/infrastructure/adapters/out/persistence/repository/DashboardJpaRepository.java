...
public interface DashboardJpaRepository extends JpaRepository<ProductEntity,Integer> {
    ...
    @Query(value = "SELECT new com.comedor.backend.infrastructure.adapters.in.web.dto.request.ProductRotationDTO(p.name, p.unit, SUM(m.quantityUsed)) " +
    ...
    List<ProductRotationDTO> findTop5ProductosMasRotados(@Param("inicio") LocalDateTime inicio, @Param("fin") LocalDateTime fin);
